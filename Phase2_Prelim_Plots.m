...
%   - 3σ RTN covariance ellipses (Batch vs Kalman)
    ...
    for i = 1:n
        ...
        rr_pre_k(i)  = rr(i) - Y_ref(2);
        rr_post_k(i) = rr(i) - Y_k(2);
    end

    %% 5. Plot helper (RR in mm/s)
    ...
    function ellipse2(P2, x0, y0, style)
        [V, D] = eig(P2);
        lam    = max(D,0);           % guard tiny negatives
        s      = 3*sqrt(diag(lam));  % 3σ lengths
